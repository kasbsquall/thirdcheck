/**
 * Deploys HardenedEscrow to CC3 testnet and records its address in .env.
 *
 *   npx hardhat run scripts/deploy-hardened.ts --network cc3
 */
...
async function main() {
  ...
  const factory = await ethers.getContractFactory("HardenedEscrow");
  const escrow = await factory.deploy();
  await escrow.waitForDeployment();
  const address = await escrow.getAddress();
  const receipt = await escrow.deploymentTransaction()!.wait();

  console.log(`  HardenedEscrow deployed`);
  ...
  setEnv("HARDENED_ESCROW_ADDRESS", address);
  console.log(`  HARDENED_ESCROW_ADDRESS written to .env\n`);
}
