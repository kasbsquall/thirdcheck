 ...
 *      and success; `bindPayment` selects the correct log among many and binds emitter, event, order,
 *      recipient and amount. Split so a non-payment consumer can reuse `verifyReceipt` and scan the
 *      returned receipt with its own predicate.
 *
 *      Usage:
